...
fn setup(
    ...
    commands.spawn((
        ...
        EnvironmentMapLight {
            diffuse_map: asset_server.load("environment/pisa_diffuse_rgb9e5_zstd.ktx2"),
            specular_map: asset_server.load("environment/pisa_specular_rgb9e5_zstd.ktx2"),
        },
    ));
}
...
fn environment_map_load_finish(
    ...
    if let Ok(environment_map) = environment_maps.get_single() {
        if asset_server.get_load_state(&environment_map.diffuse_map) == Some(LoadState::Loaded)
            && asset_server.get_load_state(&environment_map.specular_map) == Some(LoadState::Loaded)
        {
